...
import { storeOffChain } from "./Dashboard";
...
const CREDENTIAL_TYPES = [
  {
    ...
    placeholder: "License #, State, Expiration",
  },
  {
    value: "diploma",
  ...
  {
    value: "professional_cert",
    label: "Professional Certification",
    placeholder: "Cert Name, Issuing Body, ID #",
  },
  {
    value: "drivers_license",
    label: "Driver's License",
    placeholder: "License #, State, Class",
  },
  {
    value: "vaccination",
    label: "Vaccination Record",
    placeholder: "Vaccine, Date, Provider",
  },
  {
    value: "background_check",
    label: "Background Check",
    placeholder: "Check Type, Date, Result",
  },
  {
    value: "work_auth",
    label: "Work Authorization",
    placeholder: "Auth Type, Country, Expiration",
  },
];
...
function AdminView({ contract, onLog, demoMode }) {
  ...
  async function handleAddIssuer() {
    ...
    if (demoMode) {
      ...
      onLog(
        "Issuer Authorized",
        `${issuerAddr.slice(0, 8)}...${issuerAddr.slice(-4)} added`,
      );
      setIssuerAddr("");
  ...
  async function handleRemoveIssuer() {
    ...
    if (demoMode) {
      ...
      onLog(
        "Issuer Removed",
        `${issuerAddr.slice(0, 8)}...${issuerAddr.slice(-4)} removed`,
      );
      setIssuerAddr("");
      setIssuerLoading(false);
      return;
  ...
  // hash type + data, store full blob off-chain, send hash on-chain
  async function handleIssue() {
    ...
    // store the full credential blob off-chain (simulated — in prod this goes to IPFS or encrypted DB)
    // the hash is the on-chain commitment; the blob lets verifiers recompute and confirm integrity
    const blob = {
      type: credType,
      data: credData,
    ...
    storeOffChain(hash, blob);

    if (demoMode) {
      await new Promise((r) => setTimeout(r, 800));
      setCredStatus(
        `Credential issued! Hash: ${hash.slice(0, 16)}... (blob stored off-chain)`,
      );
      onLog(
        "Credential Issued",
        `${currentType.label} to ${recipientAddr.slice(0, 8)}...${recipientAddr.slice(-4)}`,
      );
      setRecipientAddr("");
      setCredData("");
  ...
  async function handleRevoke() {
    ...
    if (demoMode) {
      ...
      onLog(
        "Credential Revoked",
        `Index ${revokeIndex} for ${revokeAddr.slice(0, 8)}...${revokeAddr.slice(-4)}`,
      );
      setRevokeAddr("");
      setRevokeIndex("");
      setCredLoading(false);
    ...
    <div className="admin-view">
      <div className="view-header">
        ...
        <p className="view-desc">
          Manage trusted issuers and issue credentials. Only authorized wallets
      ...
      {activeSection === "issuers" && (
        <div className="panel">
          ...
          <p className="panel-desc">
            Authorize or revoke organizations — hospitals, universities,
          ...
          <p className="panel-desc">
            {demoMode
              ? `Try one of the demo issuers: ${Object.keys(DEMO_ISSUERS)[0]}`
              : "Enter an address to check if it is an authorized issuer."}
          </p>
          <div className="form-group">
          ...
          {checkResult !== null && (
            <div
              className={`issuer-check-result ${checkResult ? "authorized" : "not-authorized"}`}
            >
      ...
      {activeSection === "credentials" && (
        <div className="panel">
          ...
          <p className="panel-desc">
            Issue a credential to a registered user. The full credential blob is
            stored off-chain; only the keccak256 hash is written on-chain as a
          ...
          <div className="form-group">
            <label>
              ...
              <span className="field-hint">
                (stored off-chain, hashed on-chain)
              </span>
            </label>
            <input
              type="text"
